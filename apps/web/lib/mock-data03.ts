/**
 * Enhanced Mock Product Data v3
 *
 * This file extends mock-data02.ts with additional fashion attributes for:
 ...
 * - AI Look → Product Mapping (NEW)
 *
 * Total attributes: 47+ fashion-specific attributes covering:
 *
...
/**
 * Enriched mock products array with comprehensive fashion attributes (v3)
 * for advanced outfit recommendation and matching algorithms
 */
export const enhancedMockProductsV3: EnhancedProductV3[] = [
...
export function getProductsByBodyType(bodyType: BodyType): EnhancedProductV3[] {
  return enhancedMockProductsV3.filter(
    (product) =>
...
export function getProductsByDressCode(dressCode: DressCode): EnhancedProductV3[] {
  return enhancedMockProductsV3.filter((product) =>
...
export function getCapsuleWardrobeItems(): EnhancedProductV3[] {
  return enhancedMockProductsV3.filter((product) => product.capsuleWardrobe)
}
...
export function getInvestmentPieces(): EnhancedProductV3[] {
  return enhancedMockProductsV3.filter((product) => product.investmentPiece)
}
...
export function getProductsByTrendStatus(status: TrendStatus): EnhancedProductV3[] {
  return enhancedMockProductsV3.filter((product) => product.trendStatus === status)
}
...
export function searchByAiTags(
  ...
  const results = enhancedMockProductsV3.map((product) => {
    let score = 0
...
export function getProductsByVisualRole(role: VisualRole): EnhancedProductV3[] {
  return enhancedMockProductsV3.filter((product) => product.visualRole === role)
}
...
export function buildOutfitFromAiLook(
  ...
  if (footwearMatches.length === 0) {
    ...
    const heelMatches = enhancedMockProductsV3.filter(
  ...
  if (/blazer|jacket|coat|cardigan/i.test(lookDescription)) {
    ...
    if (outerwearMatches.length === 0) {
      const blazerMatches = enhancedMockProductsV3.filter(
